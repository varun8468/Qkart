  ...
  return (
    <Box display="flex" flexDirection="column">
      ...
      <Stack direction="row" my="1rem">
        ...
        <Button variant="text" onClick={() => handleView(false)}>
          Cancel
        </Button>
      </Stack>
    </Box>
...
const Checkout = () => {
  ...
  const deleteAddress = async ( addressId, token) => {
    try {
      ...
      const response = await axios.delete(config.endpoint+"/user/addresses/"+addressId, header);
  ...
  const validateRequest = (items, addresses) => {
    ...
    if(availableBalance < totalCartValue){
  ...
  const performCheckout = async (token, items, addresses) => {
    ...
    const isValidated = validateRequest(items, addresses);
    const totalCartValue = getTotalCartValue(items);
    const availableBalance = localStorage.getItem("balance");
    ...
    if(isValidated){
      try {
        ...
        if(response.status === 200){
          const balance = availableBalance - totalCartValue;
    ...
    return addresses.all.map((address) => {
      return (
        ...
        >
          ...
          <Button color="primary" variant="outlined" startIcon={<Delete />}
          onClick={()=> deleteAddress(address._id, token)}
          >
            Delete
          </Button>
        </Box>
