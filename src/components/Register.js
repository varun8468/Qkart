...
const Register = () => {
  ...
  const register = async (formData) => {
    ...
    if (validated) {
      ...
      try {
        setLoading(true);
        const response = await axios.post(config.endpoint + "/auth/register", newData);
  ...
  function render() {
    if (!loading) {
      return <Button onClick={()=>register(formData)} className="button" name="register" variant="contained">
            Register Now
          </Button>
